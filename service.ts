...
import { decidePromptUsage, type EntitlementSnapshot, type UsageDecision } from '../billing/entitlements.js';
import type {
  UserRecord,
  PlanRecord,
  PlanId,
...
const PLAN_DAILY_LIMIT: Record<PlanId, number | null> = { free: 40, pro: null, ultra: null };
const PLAN_PRICE_PKR_MONTHLY: Record<PlanId, number> = { free: 0, pro: 1000, ultra: 3000 };
const EMAIL_RE = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;

export type AuthResult =
  | { ok: true; userId: string; token: string; plan: PlanId }
  | { ok: false; reason: 'invalid_email' | 'weak_password' | 'email_taken' | 'invalid_credentials' | 'account_suspended' };

export type SessionResolution = { userId: string; role: UserRecord['role'] } | null;
...
export class IdentityService {
  constructor(private store: IdentityStore = new IdentityStore()) {}
  ...
  private audit(type: AuditEventType, userId: string | null, actorId: string | null, detail: Record<string, unknown> = {}): void {
  ...
  register(email: string, password: string): AuthResult {
    ...
    if (this.store.getUserByEmail(email)) return { ok: false, reason: 'email_taken' };
  ...
  resolveSession(rawToken: string): SessionResolution {
  ...
  getEntitlementSnapshot(userId: string): EntitlementSnapshot {
  ...
  decidePromptForUser(userId: string): UsageDecision {
  ...
  recordPromptUsage(userId: string): void {
    const day = todayUTC();
  ...
  grantPlan(
    ...
    userId: string,
    planId: PlanId,
    opts: { platform?: BillingPlatform | null; renewsAt?: string | null; expiresAt?: string | null } = {},
  ): PlanRecord {
    const plan: PlanRecord = {
      userId,
      planId,
  ...
  suspendUser(actorId: string, userId: string): void {
  ...
  createOrder(userId: string, planId: PlanId, gateway: PaymentGateway): PendingOrderRecord {
    if (planId === 'free') throw new Error('cannot_create_order_for_free_plan');
    const now = Date.now();
   ...
   */
  fulfillOrder(orderRef: string, gatewayTxnId: string): { ok: true } | { ok: false; reason: string } {
    const order = this.store.getOrder(orderRef);
    if (!order) return { ok: false, reason: 'order_not_found' };
    if (order.status === 'fulfilled') return { ok: true }; // idempotent replay
  ...
  rejectOrder(orderRef: string, reason: string): void {
    const order = this.store.getOrder(orderRef);
