 ...
 */
export function computeJazzCashHash(fields, integritySalt) {
 ...
 * in production.
 */
export function verifyJazzCashHash(fields, integritySalt) {
    if (!integritySalt)
        return false;
 ...
 */
export function buildJazzCashCheckoutFields(order, config) {
    const now = new Date();
 ...
 */
export function verifyGenericHmacSignature(rawBody, providedSignatureHex, secret) {
    if (!secret || !providedSignatureHex)
        return false;
