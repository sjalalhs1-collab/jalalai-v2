...
export interface RankedSource extends SearchHit {score:number;authority:number;relevance:number}

export class EvidenceEngine {
 rank(hits:SearchHit[],query:string):RankedSource[]{
  ...
  return hits.map((h,i)=>{
   ...
   const freshnessPenalty=0; const rankBonus=Math.max(0,1-i*.06);
   const score=Math.min(1,.50*relevance+.40*authority+.10*rankBonus-freshnessPenalty);
   return {...h,score,authority,relevance};
  }).sort((a,b)=>b.score-a.score);
 }
 fromSearch(hits:SearchHit[],claim:string):Evidence[]{return this.rank(hits,claim).slice(0,8).map(h=>({source:h.url,claim,confidence:h.score,retrievedAt:Date.now()}))}
}
